...
class _ScanResultState extends State<ScanResult> {
  final PanelController _panelController = PanelController();
    ...
    String _dishName = Controller.c.scanResult['detectionResultData']['total']['dishName'];
  @override
  void initState() {
    super.initState();
      ...
      return Row(
        children: [
          ...
          GestureDetector(
            ...
            child: Row(children: [
              Text(meal?['label']??'DINNER'.tr, style: const TextStyle(color: Colors.white,fontSize: 12)),
              SizedBox(width: 5,),
              const Icon(Icons.edit, size: 12, color: Color.fromARGB(255, 255, 255, 255))
            ],) 
          ),
          ),
        ...
        children: [
          ...
          Positioned(
            ...
            child:const CircleAvatar(
              ...
              child: Icon(AliIcon.back2,color: Colors.white,size: 30,),
            )),
          ),
          SlidingUpPanel(
      ...
      children: [
        Row(
          children: [
            ...
            SizedBox(width: 6),
            Text("${Controller.c.scanResult['detectionResultData']['total']['calories']} kcal", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ],
        ),
        const SizedBox(height: 10),
        Row(
          ...
          children: [
            _stat("CARBS".tr, Controller.c.scanResult['detectionResultData']['total']['carbs'], AliIcon.dinner4,Colors.blueAccent),
            _stat("PROTEIN".tr, Controller.c.scanResult['detectionResultData']['total']['protein'], AliIcon.meat2,Colors.redAccent),
            _stat("FATS".tr, Controller.c.scanResult['detectionResultData']['total']['fat'], AliIcon.fat,Colors.orangeAccent),
          ],
        ),
      ],
    );
  ...
  Widget _buildIngredients() {
    List<dynamic> ingredients = Controller.c.scanResult['detectionResultData']['ingredients'];
      ...
      child:   GridView.count(
        ...
        return GestureDetector(
          ...
          child:    Container(
            ...
            decoration: BoxDecoration( 
              color: meal['value']==_selectedMeal?meal['color']: Colors.white,
              borderRadius:BorderRadius.circular(10),
              border:Border.all(color:meal['color']) ),
            child: Row(
              children: [
                Icon(meal['icon'],color: meal['value']==_selectedMeal?Colors.white:meal['color']),
                const SizedBox(width: 5,),
                Text(meal['label'],style: TextStyle(color: meal['value']==_selectedMeal?Colors.white:meal['color'],fontSize: 13,fontWeight: FontWeight.w600),),
              ],
            ) 
          )
        );
